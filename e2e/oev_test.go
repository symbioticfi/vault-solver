...
func verifyOEVMetrics(t *testing.T, testEnv *testEnvironment, baseline string) {
	...
	checks := []struct {
		...
		{"solver_bot_workflow_events_total", map[string]string{"solver": "redstone-oev", "strategy": "default", "event": "auction"}},
		{"solver_bot_workflow_events_total", map[string]string{"solver": "redstone-oev", "strategy": "default", "event": "bid", "outcome": "enqueued"}},
		{"solver_bot_workflow_events_total", map[string]string{"solver": "redstone-oev", "strategy": "default", "event": "bid", "outcome": "won"}},
		{"oev_hotpath_seconds_count", map[string]string{"strategy": "default"}},
	}
	for _, check := range checks {
		initial := metricValue(baseline, check.name, check.labels)
		eventually(t, check.name+" metric", 90*time.Second, 2*time.Second, func() error {
			value := metricValue(getMetrics(t, testEnv), check.name, check.labels)
			if value-initial < 1 {
