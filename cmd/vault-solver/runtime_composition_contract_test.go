	...
	return []runtimeCompositionCase{
		{
			...
			selectionDiagnostic: `unknown 3F strategy "command-contract-missing"`,
			config: `
...
`,
		...
		{
			name:                "lifi-samechain",
			selectionDiagnostic: `unknown LI.FI strategy "command-contract-missing"`,
			config: `
orderServer:
...
`,
		...
		{
			name:                "redstone-oev",
			selectionDiagnostic: `unknown OEV strategy "command-contract-missing"`,
			config: `
...
`,
		...
		{
			name:                "rfq-filler",
			selectionDiagnostic: `unknown RFQ strategy "command-contract-missing"`,
			config: `
...
`,
		},
		{
			...
			selectionDiagnostic: `unknown UniswapX strategy "command-contract-missing"`,
			config: `
...
func TestRuntimeCommandCompositionContract(t *testing.T) {
	setCompositionEnv(t)
...
func testRuntimeCompositionCase(t *testing.T, deps app.Services, test runtimeCompositionCase) {
	t.Helper()
	entries := compositionEntries(t, []runtimeCompositionCase{test}, test.name)
	validatedRequiresTxManager, err := validateSolverConfigs(entries)
	if err != nil {
		t.Fatalf("validate configured solvers: %v", err)
	}
	constructed, constructedRequiresTxManager, err := buildIntegrations(entries, deps)
	if err != nil {
	...
	invalid := test.config + "\nstrategy: {name: command-contract-missing, config: {}}\n"
	if strings.Contains(test.config, "  name: default") {
		invalid = strings.Replace(test.config, "  name: default", "  name: command-contract-missing", 1)
	}
	invalidEntries := []config.SolverConfig{{
		Name: test.name, Config: compositionYAMLNode(t, invalid),
	}}
	_, validationErr := validateSolverConfigs(invalidEntries)
	_, _, factoryErr := buildIntegrations(invalidEntries, deps)
...
func TestConfiguredSolverAggregateRequirementContract(t *testing.T) {
	setCompositionEnv(t)
	deps := newCompositionDeps(t)
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			validatedRequiresManager, err := validateSolverConfigs(entries)
			if err != nil {
				t.Fatalf("validate configured solvers: %v", err)
			}
			constructed, constructedRequiresManager, err := buildIntegrations(entries, deps)
			if err != nil {
				t.Fatalf("construct configured solvers: %v", err)
			}
...
func compositionEntries(
	...
	for _, name := range names {
		...
		if index < 0 {
			t.Fatalf("missing contract config for %q", name)
		}
...
func solverNames(solvers []app.Integration) []string {
...
func TestUnknownSolverDiagnosticContract(t *testing.T) {
	...
	checks := []struct {
		...
		{name: "runtime construction", run: func() error {
			_, _, err := buildIntegrations(entries, app.Services{})
			return err
		}},
		{name: "offline validation", run: func() error {
			_, err := validateSolverConfigs(entries)
			return err
		}},
	}
...
func newCompositionDeps(t *testing.T) app.Services {
	t.Helper()
	rpcServer := newCompositionRPCServer(t)
