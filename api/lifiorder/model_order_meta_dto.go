/*
Lifi Intents API Reference

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.0.20
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package lifiorder

import (
	"encoding/json"
)

// checks if the OrderMetaDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &OrderMetaDto{}

// OrderMetaDto struct for OrderMetaDto
type OrderMetaDto struct {
	// Timestamp when the order was submitted in seconds
	SubmitTime float32 `json:"submitTime"`
	// Current status of the order
	OrderStatus string `json:"orderStatus"`
	// Unique identifier for the order (intent id)
	OrderIdentifier string `json:"orderIdentifier"`
	// On-chain order ID
	OnChainOrderId string `json:"onChainOrderId"`
	// Parsed destination address of the order
	DestinationAddress string `json:"destinationAddress"`
	// Token the user sends on the origin chain, decoded from the intent input (inputs[0]) and rendered in the chain-native form. Only the first leg is represented; an order with further inputs or outputs does not report them here. Null when the leg is missing or the encoding cannot be decoded. A native asset is reported as the native sentinel of that chain, in that same form: the zero address on EVM, its base58check equivalent T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb on Tron, and 11111111111111111111111111111111 on Solana. When the order is refunded, this is also the token returned to the user.
	SendingTokenAddress NullableString `json:"sendingTokenAddress"`
	// Amount of the sending token, in the smallest unit of that token (inputs[0]). Only the first leg is represented; an order with further inputs or outputs does not report them here. Null when the leg is missing or the amount cannot be read. When the order is refunded, this is also the amount returned to the user.
	SendingTokenAmount NullableString `json:"sendingTokenAmount"`
	// Token the user receives on the destination chain, decoded from the intent output (outputs[0]) and rendered in the chain-native form. Only the first leg is represented; an order with further inputs or outputs does not report them here. Null when the leg is missing or the encoding cannot be decoded. A native asset is reported as the native sentinel of that chain, in that same form: the zero address on EVM, its base58check equivalent T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb on Tron, and 11111111111111111111111111111111 on Solana.
	ReceivingTokenAddress NullableString `json:"receivingTokenAddress"`
	// Amount encoded on outputs[0], in the smallest unit of the receiving token. Only the first leg is represented; an order with further inputs or outputs does not report them here. For limit orders this is exactly the amount the intent requires to be delivered. For DutchAuction and ExclusiveDutchAuction orders it is the minimum amount: the output settler requires this plus the remaining auction decay, so the amount required at fill time is higher. Never the amount actually delivered, which is not stored. Null when the leg is missing or the amount cannot be read.
	ReceivingTokenAmount NullableString `json:"receivingTokenAmount"`
	// Transaction hash when order was initiated (on-chain order) [eg: Open escrow event]
	OrderInitiatedTxHash NullableString `json:"orderInitiatedTxHash"`
	// Transaction hash of the OutputFilled event
	OrderDeliveredTxHash NullableString `json:"orderDeliveredTxHash"`
	// Transaction hash of the OutputProven event
	OrderVerifiedTxHash NullableString `json:"orderVerifiedTxHash"`
	// Transaction hash of the Finalised event
	OrderSettledTxHash NullableString `json:"orderSettledTxHash"`
	// Transaction hash of the Refunded event
	RefundTxHash NullableString `json:"refundTxHash"`
	// Date when the order was signed
	SignedAt NullableString `json:"signedAt"`
	// Date when the order expires
	ExpiredAt NullableString `json:"expiredAt"`
	// Date when the order was delivered
	DeliveredAt NullableString `json:"deliveredAt"`
	// Date when the order was settled
	SettledAt NullableString `json:"settledAt"`
	// Date when the order was refunded
	RefundedAt NullableString `json:"refundedAt"`
	// Last compact deposit block number
	LastCompactDepositBlockNumber NullableString `json:"lastCompactDepositBlockNumber"`
	// Quote ID associated with the order
	QuoteId NullableString `json:"quoteId"`
	// Solver address that filled the order
	SolverAddress NullableString `json:"solverAddress,omitempty"`
	// Integrator key hash identifying the integrator this order belongs to
	IntegratorKeyHash    *string `json:"integratorKeyHash,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _OrderMetaDto OrderMetaDto

// NewOrderMetaDto instantiates a new OrderMetaDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewOrderMetaDto(submitTime float32, orderStatus string, orderIdentifier string, onChainOrderId string, destinationAddress string, sendingTokenAddress NullableString, sendingTokenAmount NullableString, receivingTokenAddress NullableString, receivingTokenAmount NullableString, orderInitiatedTxHash NullableString, orderDeliveredTxHash NullableString, orderVerifiedTxHash NullableString, orderSettledTxHash NullableString, refundTxHash NullableString, signedAt NullableString, expiredAt NullableString, deliveredAt NullableString, settledAt NullableString, refundedAt NullableString, lastCompactDepositBlockNumber NullableString, quoteId NullableString) *OrderMetaDto {
	this := OrderMetaDto{}
	this.SubmitTime = submitTime
	this.OrderStatus = orderStatus
	this.OrderIdentifier = orderIdentifier
	this.OnChainOrderId = onChainOrderId
	this.DestinationAddress = destinationAddress
	this.SendingTokenAddress = sendingTokenAddress
	this.SendingTokenAmount = sendingTokenAmount
	this.ReceivingTokenAddress = receivingTokenAddress
	this.ReceivingTokenAmount = receivingTokenAmount
	this.OrderInitiatedTxHash = orderInitiatedTxHash
	this.OrderDeliveredTxHash = orderDeliveredTxHash
	this.OrderVerifiedTxHash = orderVerifiedTxHash
	this.OrderSettledTxHash = orderSettledTxHash
	this.RefundTxHash = refundTxHash
	this.SignedAt = signedAt
	this.ExpiredAt = expiredAt
	this.DeliveredAt = deliveredAt
	this.SettledAt = settledAt
	this.RefundedAt = refundedAt
	this.LastCompactDepositBlockNumber = lastCompactDepositBlockNumber
	this.QuoteId = quoteId
	return &this
}

// NewOrderMetaDtoWithDefaults instantiates a new OrderMetaDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewOrderMetaDtoWithDefaults() *OrderMetaDto {
	this := OrderMetaDto{}
	return &this
}

// GetSubmitTime returns the SubmitTime field value
func (o *OrderMetaDto) GetSubmitTime() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.SubmitTime
}

// GetSubmitTimeOk returns a tuple with the SubmitTime field value
// and a boolean to check if the value has been set.
func (o *OrderMetaDto) GetSubmitTimeOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SubmitTime, true
}

// SetSubmitTime sets field value
func (o *OrderMetaDto) SetSubmitTime(v float32) {
	o.SubmitTime = v
}

// GetOrderStatus returns the OrderStatus field value
func (o *OrderMetaDto) GetOrderStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OrderStatus
}

// GetOrderStatusOk returns a tuple with the OrderStatus field value
// and a boolean to check if the value has been set.
func (o *OrderMetaDto) GetOrderStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrderStatus, true
}

// SetOrderStatus sets field value
func (o *OrderMetaDto) SetOrderStatus(v string) {
	o.OrderStatus = v
}

// GetOrderIdentifier returns the OrderIdentifier field value
func (o *OrderMetaDto) GetOrderIdentifier() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OrderIdentifier
}

// GetOrderIdentifierOk returns a tuple with the OrderIdentifier field value
// and a boolean to check if the value has been set.
func (o *OrderMetaDto) GetOrderIdentifierOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrderIdentifier, true
}

// SetOrderIdentifier sets field value
func (o *OrderMetaDto) SetOrderIdentifier(v string) {
	o.OrderIdentifier = v
}

// GetOnChainOrderId returns the OnChainOrderId field value
func (o *OrderMetaDto) GetOnChainOrderId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OnChainOrderId
}

// GetOnChainOrderIdOk returns a tuple with the OnChainOrderId field value
// and a boolean to check if the value has been set.
func (o *OrderMetaDto) GetOnChainOrderIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OnChainOrderId, true
}

// SetOnChainOrderId sets field value
func (o *OrderMetaDto) SetOnChainOrderId(v string) {
	o.OnChainOrderId = v
}

// GetDestinationAddress returns the DestinationAddress field value
func (o *OrderMetaDto) GetDestinationAddress() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.DestinationAddress
}

// GetDestinationAddressOk returns a tuple with the DestinationAddress field value
// and a boolean to check if the value has been set.
func (o *OrderMetaDto) GetDestinationAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DestinationAddress, true
}

// SetDestinationAddress sets field value
func (o *OrderMetaDto) SetDestinationAddress(v string) {
	o.DestinationAddress = v
}

// GetSendingTokenAddress returns the SendingTokenAddress field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetSendingTokenAddress() string {
	if o == nil || o.SendingTokenAddress.Get() == nil {
		var ret string
		return ret
	}

	return *o.SendingTokenAddress.Get()
}

// GetSendingTokenAddressOk returns a tuple with the SendingTokenAddress field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetSendingTokenAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SendingTokenAddress.Get(), o.SendingTokenAddress.IsSet()
}

// SetSendingTokenAddress sets field value
func (o *OrderMetaDto) SetSendingTokenAddress(v string) {
	o.SendingTokenAddress.Set(&v)
}

// GetSendingTokenAmount returns the SendingTokenAmount field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetSendingTokenAmount() string {
	if o == nil || o.SendingTokenAmount.Get() == nil {
		var ret string
		return ret
	}

	return *o.SendingTokenAmount.Get()
}

// GetSendingTokenAmountOk returns a tuple with the SendingTokenAmount field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetSendingTokenAmountOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SendingTokenAmount.Get(), o.SendingTokenAmount.IsSet()
}

// SetSendingTokenAmount sets field value
func (o *OrderMetaDto) SetSendingTokenAmount(v string) {
	o.SendingTokenAmount.Set(&v)
}

// GetReceivingTokenAddress returns the ReceivingTokenAddress field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetReceivingTokenAddress() string {
	if o == nil || o.ReceivingTokenAddress.Get() == nil {
		var ret string
		return ret
	}

	return *o.ReceivingTokenAddress.Get()
}

// GetReceivingTokenAddressOk returns a tuple with the ReceivingTokenAddress field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetReceivingTokenAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ReceivingTokenAddress.Get(), o.ReceivingTokenAddress.IsSet()
}

// SetReceivingTokenAddress sets field value
func (o *OrderMetaDto) SetReceivingTokenAddress(v string) {
	o.ReceivingTokenAddress.Set(&v)
}

// GetReceivingTokenAmount returns the ReceivingTokenAmount field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetReceivingTokenAmount() string {
	if o == nil || o.ReceivingTokenAmount.Get() == nil {
		var ret string
		return ret
	}

	return *o.ReceivingTokenAmount.Get()
}

// GetReceivingTokenAmountOk returns a tuple with the ReceivingTokenAmount field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetReceivingTokenAmountOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ReceivingTokenAmount.Get(), o.ReceivingTokenAmount.IsSet()
}

// SetReceivingTokenAmount sets field value
func (o *OrderMetaDto) SetReceivingTokenAmount(v string) {
	o.ReceivingTokenAmount.Set(&v)
}

// GetOrderInitiatedTxHash returns the OrderInitiatedTxHash field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetOrderInitiatedTxHash() string {
	if o == nil || o.OrderInitiatedTxHash.Get() == nil {
		var ret string
		return ret
	}

	return *o.OrderInitiatedTxHash.Get()
}

// GetOrderInitiatedTxHashOk returns a tuple with the OrderInitiatedTxHash field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetOrderInitiatedTxHashOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OrderInitiatedTxHash.Get(), o.OrderInitiatedTxHash.IsSet()
}

// SetOrderInitiatedTxHash sets field value
func (o *OrderMetaDto) SetOrderInitiatedTxHash(v string) {
	o.OrderInitiatedTxHash.Set(&v)
}

// GetOrderDeliveredTxHash returns the OrderDeliveredTxHash field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetOrderDeliveredTxHash() string {
	if o == nil || o.OrderDeliveredTxHash.Get() == nil {
		var ret string
		return ret
	}

	return *o.OrderDeliveredTxHash.Get()
}

// GetOrderDeliveredTxHashOk returns a tuple with the OrderDeliveredTxHash field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetOrderDeliveredTxHashOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OrderDeliveredTxHash.Get(), o.OrderDeliveredTxHash.IsSet()
}

// SetOrderDeliveredTxHash sets field value
func (o *OrderMetaDto) SetOrderDeliveredTxHash(v string) {
	o.OrderDeliveredTxHash.Set(&v)
}

// GetOrderVerifiedTxHash returns the OrderVerifiedTxHash field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetOrderVerifiedTxHash() string {
	if o == nil || o.OrderVerifiedTxHash.Get() == nil {
		var ret string
		return ret
	}

	return *o.OrderVerifiedTxHash.Get()
}

// GetOrderVerifiedTxHashOk returns a tuple with the OrderVerifiedTxHash field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetOrderVerifiedTxHashOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OrderVerifiedTxHash.Get(), o.OrderVerifiedTxHash.IsSet()
}

// SetOrderVerifiedTxHash sets field value
func (o *OrderMetaDto) SetOrderVerifiedTxHash(v string) {
	o.OrderVerifiedTxHash.Set(&v)
}

// GetOrderSettledTxHash returns the OrderSettledTxHash field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetOrderSettledTxHash() string {
	if o == nil || o.OrderSettledTxHash.Get() == nil {
		var ret string
		return ret
	}

	return *o.OrderSettledTxHash.Get()
}

// GetOrderSettledTxHashOk returns a tuple with the OrderSettledTxHash field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetOrderSettledTxHashOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OrderSettledTxHash.Get(), o.OrderSettledTxHash.IsSet()
}

// SetOrderSettledTxHash sets field value
func (o *OrderMetaDto) SetOrderSettledTxHash(v string) {
	o.OrderSettledTxHash.Set(&v)
}

// GetRefundTxHash returns the RefundTxHash field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetRefundTxHash() string {
	if o == nil || o.RefundTxHash.Get() == nil {
		var ret string
		return ret
	}

	return *o.RefundTxHash.Get()
}

// GetRefundTxHashOk returns a tuple with the RefundTxHash field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetRefundTxHashOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.RefundTxHash.Get(), o.RefundTxHash.IsSet()
}

// SetRefundTxHash sets field value
func (o *OrderMetaDto) SetRefundTxHash(v string) {
	o.RefundTxHash.Set(&v)
}

// GetSignedAt returns the SignedAt field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetSignedAt() string {
	if o == nil || o.SignedAt.Get() == nil {
		var ret string
		return ret
	}

	return *o.SignedAt.Get()
}

// GetSignedAtOk returns a tuple with the SignedAt field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetSignedAtOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SignedAt.Get(), o.SignedAt.IsSet()
}

// SetSignedAt sets field value
func (o *OrderMetaDto) SetSignedAt(v string) {
	o.SignedAt.Set(&v)
}

// GetExpiredAt returns the ExpiredAt field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetExpiredAt() string {
	if o == nil || o.ExpiredAt.Get() == nil {
		var ret string
		return ret
	}

	return *o.ExpiredAt.Get()
}

// GetExpiredAtOk returns a tuple with the ExpiredAt field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetExpiredAtOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ExpiredAt.Get(), o.ExpiredAt.IsSet()
}

// SetExpiredAt sets field value
func (o *OrderMetaDto) SetExpiredAt(v string) {
	o.ExpiredAt.Set(&v)
}

// GetDeliveredAt returns the DeliveredAt field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetDeliveredAt() string {
	if o == nil || o.DeliveredAt.Get() == nil {
		var ret string
		return ret
	}

	return *o.DeliveredAt.Get()
}

// GetDeliveredAtOk returns a tuple with the DeliveredAt field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetDeliveredAtOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.DeliveredAt.Get(), o.DeliveredAt.IsSet()
}

// SetDeliveredAt sets field value
func (o *OrderMetaDto) SetDeliveredAt(v string) {
	o.DeliveredAt.Set(&v)
}

// GetSettledAt returns the SettledAt field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetSettledAt() string {
	if o == nil || o.SettledAt.Get() == nil {
		var ret string
		return ret
	}

	return *o.SettledAt.Get()
}

// GetSettledAtOk returns a tuple with the SettledAt field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetSettledAtOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SettledAt.Get(), o.SettledAt.IsSet()
}

// SetSettledAt sets field value
func (o *OrderMetaDto) SetSettledAt(v string) {
	o.SettledAt.Set(&v)
}

// GetRefundedAt returns the RefundedAt field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetRefundedAt() string {
	if o == nil || o.RefundedAt.Get() == nil {
		var ret string
		return ret
	}

	return *o.RefundedAt.Get()
}

// GetRefundedAtOk returns a tuple with the RefundedAt field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetRefundedAtOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.RefundedAt.Get(), o.RefundedAt.IsSet()
}

// SetRefundedAt sets field value
func (o *OrderMetaDto) SetRefundedAt(v string) {
	o.RefundedAt.Set(&v)
}

// GetLastCompactDepositBlockNumber returns the LastCompactDepositBlockNumber field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetLastCompactDepositBlockNumber() string {
	if o == nil || o.LastCompactDepositBlockNumber.Get() == nil {
		var ret string
		return ret
	}

	return *o.LastCompactDepositBlockNumber.Get()
}

// GetLastCompactDepositBlockNumberOk returns a tuple with the LastCompactDepositBlockNumber field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetLastCompactDepositBlockNumberOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.LastCompactDepositBlockNumber.Get(), o.LastCompactDepositBlockNumber.IsSet()
}

// SetLastCompactDepositBlockNumber sets field value
func (o *OrderMetaDto) SetLastCompactDepositBlockNumber(v string) {
	o.LastCompactDepositBlockNumber.Set(&v)
}

// GetQuoteId returns the QuoteId field value
// If the value is explicit nil, the zero value for string will be returned
func (o *OrderMetaDto) GetQuoteId() string {
	if o == nil || o.QuoteId.Get() == nil {
		var ret string
		return ret
	}

	return *o.QuoteId.Get()
}

// GetQuoteIdOk returns a tuple with the QuoteId field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetQuoteIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.QuoteId.Get(), o.QuoteId.IsSet()
}

// SetQuoteId sets field value
func (o *OrderMetaDto) SetQuoteId(v string) {
	o.QuoteId.Set(&v)
}

// GetSolverAddress returns the SolverAddress field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *OrderMetaDto) GetSolverAddress() string {
	if o == nil || IsNil(o.SolverAddress.Get()) {
		var ret string
		return ret
	}
	return *o.SolverAddress.Get()
}

// GetSolverAddressOk returns a tuple with the SolverAddress field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *OrderMetaDto) GetSolverAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SolverAddress.Get(), o.SolverAddress.IsSet()
}

// HasSolverAddress returns a boolean if a field has been set.
func (o *OrderMetaDto) HasSolverAddress() bool {
	if o != nil && o.SolverAddress.IsSet() {
		return true
	}

	return false
}

// SetSolverAddress gets a reference to the given NullableString and assigns it to the SolverAddress field.
func (o *OrderMetaDto) SetSolverAddress(v string) {
	o.SolverAddress.Set(&v)
}

// SetSolverAddressNil sets the value for SolverAddress to be an explicit nil
func (o *OrderMetaDto) SetSolverAddressNil() {
	o.SolverAddress.Set(nil)
}

// UnsetSolverAddress ensures that no value is present for SolverAddress, not even an explicit nil
func (o *OrderMetaDto) UnsetSolverAddress() {
	o.SolverAddress.Unset()
}

// GetIntegratorKeyHash returns the IntegratorKeyHash field value if set, zero value otherwise.
func (o *OrderMetaDto) GetIntegratorKeyHash() string {
	if o == nil || IsNil(o.IntegratorKeyHash) {
		var ret string
		return ret
	}
	return *o.IntegratorKeyHash
}

// GetIntegratorKeyHashOk returns a tuple with the IntegratorKeyHash field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *OrderMetaDto) GetIntegratorKeyHashOk() (*string, bool) {
	if o == nil || IsNil(o.IntegratorKeyHash) {
		return nil, false
	}
	return o.IntegratorKeyHash, true
}

// HasIntegratorKeyHash returns a boolean if a field has been set.
func (o *OrderMetaDto) HasIntegratorKeyHash() bool {
	if o != nil && !IsNil(o.IntegratorKeyHash) {
		return true
	}

	return false
}

// SetIntegratorKeyHash gets a reference to the given string and assigns it to the IntegratorKeyHash field.
func (o *OrderMetaDto) SetIntegratorKeyHash(v string) {
	o.IntegratorKeyHash = &v
}

func (o OrderMetaDto) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o OrderMetaDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["submitTime"] = o.SubmitTime
	toSerialize["orderStatus"] = o.OrderStatus
	toSerialize["orderIdentifier"] = o.OrderIdentifier
	toSerialize["onChainOrderId"] = o.OnChainOrderId
	toSerialize["destinationAddress"] = o.DestinationAddress
	toSerialize["sendingTokenAddress"] = o.SendingTokenAddress.Get()
	toSerialize["sendingTokenAmount"] = o.SendingTokenAmount.Get()
	toSerialize["receivingTokenAddress"] = o.ReceivingTokenAddress.Get()
	toSerialize["receivingTokenAmount"] = o.ReceivingTokenAmount.Get()
	toSerialize["orderInitiatedTxHash"] = o.OrderInitiatedTxHash.Get()
	toSerialize["orderDeliveredTxHash"] = o.OrderDeliveredTxHash.Get()
	toSerialize["orderVerifiedTxHash"] = o.OrderVerifiedTxHash.Get()
	toSerialize["orderSettledTxHash"] = o.OrderSettledTxHash.Get()
	toSerialize["refundTxHash"] = o.RefundTxHash.Get()
	toSerialize["signedAt"] = o.SignedAt.Get()
	toSerialize["expiredAt"] = o.ExpiredAt.Get()
	toSerialize["deliveredAt"] = o.DeliveredAt.Get()
	toSerialize["settledAt"] = o.SettledAt.Get()
	toSerialize["refundedAt"] = o.RefundedAt.Get()
	toSerialize["lastCompactDepositBlockNumber"] = o.LastCompactDepositBlockNumber.Get()
	toSerialize["quoteId"] = o.QuoteId.Get()
	if o.SolverAddress.IsSet() {
		toSerialize["solverAddress"] = o.SolverAddress.Get()
	}
	if !IsNil(o.IntegratorKeyHash) {
		toSerialize["integratorKeyHash"] = o.IntegratorKeyHash
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *OrderMetaDto) UnmarshalJSON(data []byte) (err error) {
	// Required-property validation removed by hack/openapi-relax-client.py:
	// upstream may drop fields at any time; absent values zero-value instead of
	// failing the whole decode.

	varOrderMetaDto := _OrderMetaDto{}

	err = json.Unmarshal(data, &varOrderMetaDto)

	if err != nil {
		return err
	}

	*o = OrderMetaDto(varOrderMetaDto)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "submitTime")
		delete(additionalProperties, "orderStatus")
		delete(additionalProperties, "orderIdentifier")
		delete(additionalProperties, "onChainOrderId")
		delete(additionalProperties, "destinationAddress")
		delete(additionalProperties, "sendingTokenAddress")
		delete(additionalProperties, "sendingTokenAmount")
		delete(additionalProperties, "receivingTokenAddress")
		delete(additionalProperties, "receivingTokenAmount")
		delete(additionalProperties, "orderInitiatedTxHash")
		delete(additionalProperties, "orderDeliveredTxHash")
		delete(additionalProperties, "orderVerifiedTxHash")
		delete(additionalProperties, "orderSettledTxHash")
		delete(additionalProperties, "refundTxHash")
		delete(additionalProperties, "signedAt")
		delete(additionalProperties, "expiredAt")
		delete(additionalProperties, "deliveredAt")
		delete(additionalProperties, "settledAt")
		delete(additionalProperties, "refundedAt")
		delete(additionalProperties, "lastCompactDepositBlockNumber")
		delete(additionalProperties, "quoteId")
		delete(additionalProperties, "solverAddress")
		delete(additionalProperties, "integratorKeyHash")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableOrderMetaDto struct {
	value *OrderMetaDto
	isSet bool
}

func (v NullableOrderMetaDto) Get() *OrderMetaDto {
	return v.value
}

func (v *NullableOrderMetaDto) Set(val *OrderMetaDto) {
	v.value = val
	v.isSet = true
}

func (v NullableOrderMetaDto) IsSet() bool {
	return v.isSet
}

func (v *NullableOrderMetaDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOrderMetaDto(val *OrderMetaDto) *NullableOrderMetaDto {
	return &NullableOrderMetaDto{value: val, isSet: true}
}

func (v NullableOrderMetaDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOrderMetaDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
