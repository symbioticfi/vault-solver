...
# KNOWN BREAKAGE (pre-existing, not caused by the tolerant flags): openapi-generator 7.24.0
# emits `nil` as a Go type for this spec's `cancel: {type: null}` (OpenAPI 3.1), which does not
# compile. The committed client was generated by an older generator and has `interface{}` there,
# so it is currently maintained in place: hack/openapi-relax-client.py strips its
# required-property checks and DisallowUnknownFields, leaving it as tolerant as the others.
# Regenerating needs a generator version that handles `type: null`, or a spec fix upstream.
