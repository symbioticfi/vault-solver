...
func TestValidateFillRoutesCanonicalizesAndChecksCapacity(t *testing.T) {
	...
	testcheck.NoError(t, err, "validate: %v")
...
func cloneFillRoute(route FillRoute) FillRoute {
	route.AmountIn = bigmath.Clone(route.AmountIn)
	route.ExpectedAmountOut = bigmath.Clone(route.ExpectedAmountOut)
	route.MinAmountOut = bigmath.Clone(route.MinAmountOut)
	route.ReservedAmountOut = bigmath.Clone(route.ReservedAmountOut)
