...
func TestFillGasCostIncludesSettlementRouteAndPrivateOverhead(t *testing.T) {
	...
	testcheck.NoError(t, err, "direct FillGasCost: %v")
	...
	testcheck.NoError(t, err, "private FillGasCost: %v")
...
func TestGasPricingCeilingNeverWrapsAtUint64Boundary(t *testing.T) {
	token := common.Address{1}
	prices := liquidlanegas.NewPriceSnapshot(map[common.Address]*big.Int{token: big.NewInt(nativeUnit)})
	pricing, err := NewGasPricing(big.NewInt(2), token, prices, nil, 0, GasEnvelope{PrivateRouteUnits: math.MaxUint64})
	testcheck.NoError(t, err)
	want := new(big.Int).Lsh(new(big.Int).SetUint64(math.MaxUint64), 1)
	for _, count := range []int{1, 2, math.MaxInt} {
		if got := pricing.MaxCost(count, count); got.Cmp(want) != 0 {
			t.Fatalf("%d routes reduced saturated gas cost: got %s, want %s", count, got, want)
		}
	}
}
