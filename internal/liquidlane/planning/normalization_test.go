...
func TestNormalizeFixedInventoryOwnsBufferedCandidates(t *testing.T) {
	t.Parallel()
	for _, tc := range []struct {
		...
		{"direct", false, 100, 100}, {"private", true, 100, 99},
		{"private rounds to zero", true, 1, 0}, {"empty", false, 0, 0},
	} {
		t.Run(tc.name, func(t *testing.T) {
			item := liquidlane.Inventory{Route: liquidlane.Route{ID: "route", TokenInDecimals: 6, TokenOutDecimals: 6},
				MaxAssets: big.NewInt(tc.capacity), MaxRate: big.NewInt(1_000_000_000_000_000_000)}
			id := common.HexToHash("0x1")
			if tc.private {
				item.DiscountID = &id
			}
			got := NormalizeFixedInventory([]liquidlane.Inventory{item}, 100)
			if tc.want == 0 {
				if len(got) != 0 {
					t.Fatalf("unusable inventory produced %v", got)
				}
				return
			}
			if len(got) != 1 || got[0].MaxAmountIn.Cmp(big.NewInt(tc.want)) != 0 || got[0].MaxAmountOut.Cmp(big.NewInt(tc.want)) != 0 {
				t.Fatalf("candidates = %+v, want capacity %d", got, tc.want)
			}
			got[0].MaxAmountOut.SetInt64(0)
			got[0].Rate.SetInt64(0)
			if tc.private {
				*got[0].DiscountID = common.Hash{}
			}
			if item.MaxAssets.Cmp(big.NewInt(tc.capacity)) != 0 || item.MaxRate.Cmp(big.NewInt(1_000_000_000_000_000_000)) != 0 || id != common.HexToHash("0x1") {
				t.Fatal("candidate mutation changed its source inventory")
			}
		})
	}
}
