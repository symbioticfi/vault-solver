...
func TestAllocateExactInputUsesCoveringAlternative(t *testing.T) {
	...
	result := NewQuotePool([]Candidate{private, direct}).allocateExactInputWithPolicy(big.NewInt(80), 1, false)
...
func TestAllocateExactInputDoesNotReusePhysicalRoute(t *testing.T) {
	...
	result := NewQuotePool([]Candidate{private, direct}).allocateExactInputWithPolicy(big.NewInt(120), 2, false)
...
func TestAllocateExactInputPrefersDirectCandidateOnTie(t *testing.T) {
	...
	result := NewQuotePool([]Candidate{private, direct}).allocateExactInputWithPolicy(big.NewInt(50), 1, false)
...
func TestAllocateExactInputChoosesBestCoveringRouteBeforeApplyingLimit(t *testing.T) {
	...
	result := NewQuotePool([]Candidate{narrowPrivate, wideDirect, betterCovering}).
...
func TestAllocateExactInputRejectsInvalidRequestWithoutNilAmounts(t *testing.T) {
	result := NewQuotePool(nil).allocateExactInputWithPolicy(nil, 0, false)
...
func FuzzAllocateExactInputInvariants(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, rawAmount uint64, rawRoutes uint8, rawCapacity uint64, rawRate uint64) {
		...
		result := NewQuotePool(candidates).allocateExactInputWithPolicy(amount, maxRoutes, false)
