...
func (a *FillSolution) Finalize(requiredAmountOut *big.Int) []liquidlane.PlanLeg {
	...
	routes := make([]liquidlane.PlanLeg, len(a.routes))
	for index, leg := range a.routes {
		routes[index] = liquidlane.PlanLeg{
...
func SolveFill(task FillTask) (*FillSolution, error) {
	...
	candidates, err := eligibleFills(task)
	...
	for index, leg := range allocation {
		allocation[index].targetOutput = new(big.Int).Sub(
			...
			scaleUp(leg.executableOutput, task.PriceBufferBps),
		)
...
type fillRouteGroup struct {
	id           liquidlane.RouteID
	alternatives []pricedFill
}

type fillAllocation struct {
	candidate        pricedFill
	amountIn         *big.Int
...
func (candidate pricedFill) id() liquidlane.CandidateID {
	return liquidlane.NewCandidateID(candidate.quote.Route, candidate.quote.DiscountID)
}

func eligibleFills(task FillTask) ([]pricedFill, error) {
	seen := make(map[liquidlane.CandidateID]struct{}, len(task.Quotes))
	candidates := make([]pricedFill, 0, len(task.Quotes))
	for _, quote := range task.Quotes {
		...
		candidate := pricedFill{quote: quote, capacity: capacity}
		candidate.maxInput = maxInputWithinCapacity(
			candidate, task.AmountIn, capacity, task.PriceBufferBps,
		)
		candidateID := candidate.id()
		_, duplicate := seen[candidateID]
		if candidate.maxInput.Sign() <= 0 || duplicate {
			continue
		}
...
func allocateFill(
	candidates []pricedFill,
	amountIn *big.Int,
	maxRoutes int,
	priceBufferBps int,
	inputPolicy UncoveredInputPolicy,
) []fillAllocation {
	routes := groupFillsByRoute(candidates)
...
func groupFillsByRoute(candidates []pricedFill) []fillRouteGroup {
	byRoute := make(map[liquidlane.RouteID][]pricedFill)
	for _, candidate := range candidates {
		byRoute[candidate.quote.ID] = append(byRoute[candidate.quote.ID], candidate)
	}
	routeIDs := make([]liquidlane.RouteID, 0, len(byRoute))
	for routeID := range byRoute {
		routeIDs = append(routeIDs, routeID)
	}
	slices.Sort(routeIDs)
	routes := make([]fillRouteGroup, 0, len(byRoute))
	for _, routeID := range routeIDs {
		routes = append(routes, fillRouteGroup{id: routeID, alternatives: byRoute[routeID]})
	}
	return routes
}

func fillCandidateBetter(left, right pricedFill) bool {
...
func fillCapacityLimits(candidates []pricedFill) map[liquidlane.CapacityID]*big.Int {
...
func fillRouteChoice(
	...
	for index, candidate := range route.alternatives {
		candidateCapacity := cloneMin(capacityLeft, candidate.capacity)
...
func reservedCapacityOutput(
	...
	buffer := scaleUp(amountOut, priceBufferBps)
