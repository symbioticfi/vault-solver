...
func TestDistributeMinimumsAcrossRoundingBoundaries(t *testing.T) {
	for a := int64(1); a <= 8; a++ {
		for b := int64(1); b <= 8; b++ {
			targets := []*big.Int{big.NewInt(a), big.NewInt(b), big.NewInt(1)}
			for total := int64(0); total <= a+b+2; total++ {
				got := distributeMinimums(targets, big.NewInt(total))
				if total < 3 || total > a+b+1 {
					if got != nil {
						t.Fatalf("accepted impossible total %d for %v", total, targets)
					}
					continue
				}
				if len(got) != len(targets) {
					t.Fatalf("total %d for %v did not distribute", total, targets)
				}
				sum := new(big.Int)
				for i, value := range got {
					if value.Sign() <= 0 || value.Cmp(targets[i]) > 0 {
						t.Fatalf("allocation %v exceeds positive targets %v", got, targets)
					}
					sum.Add(sum, value)
				}
				if sum.Int64() != total {
					t.Fatalf("allocation %v sums to %s, want %d", got, sum, total)
				}
			}
		}
	}
...
func TestSolveFillRejectsQuoteForAnotherInputAmount(t *testing.T) {
	in, out := common.Address{19: 1}, common.Address{19: 2}
	quote := testFillQuote("route", "capacity", in, out, 99, 100, 100, nil)
	if solution, err := SolveFill(FillTask{TokenIn: in, TokenOut: out, AmountIn: big.NewInt(100),
		Quotes: []liquidlane.FillQuote{quote}, MaxRoutes: 1}); solution != nil || err == nil {
		t.Fatalf("accepted quote for another input: solution=%v err=%v", solution, err)
	}
}
