...
func TestParseSignedBoundsDecimalNonce(t *testing.T) {
	valid := new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 256), big.NewInt(1)).String()
	invalid := []string{"", "0x2", "+2", "-2", "2.0", new(big.Int).Lsh(big.NewInt(1), 256).String()}
	for _, nonce := range append([]string{valid}, invalid...) {
		resolved := validSignedDiscount(nonce)
		_, err := ParseSigned(resolved)
		if nonce == valid && err != nil {
			t.Fatalf("maximum uint256 nonce: %v", err)
		}
		if nonce != valid && err == nil {
...
func validSignedDiscount(nonce string) *Resolved {
	return &Resolved{
		DiscountID: "0x" + hash64,
		Discount: Terms{
			Adapter: "0x0000000000000000000000000000000000000abc", TokenToRedeem: "0x0000000000000000000000000000000000000def",
			Discount: "0", Signer: "0x0000000000000000000000000000000000000aaa",
			Protocol: "0x0000000000000000000000000000000000000bbb", Nonce: nonce, Deadline: 1_900_000_000,
		},
		SignerSignature: "0x01", ProtocolDeadline: 1_900_000_001, ProtocolSignature: "0x02",
	}
}
