...
type characterizationStrategy struct {
	mu     sync.Mutex
	calls  int
	inputs []strategytypes.OfferInput
	output strategytypes.OfferOutput
	err    error
}

func (s *characterizationStrategy) DecideOffers(_ context.Context, input strategytypes.OfferInput) (strategytypes.OfferOutput, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.calls++
...
func (s *characterizationStrategy) snapshot() (int, []strategytypes.OfferInput) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.calls, append([]strategytypes.OfferInput(nil), s.inputs...)
}

type characterizationTrace struct {
	mu      sync.Mutex
	events  []string
	logs    []string
	posts   []string
...
func (t *characterizationTrace) appendLog(entry string) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *characterizationTrace) snapshot() (events, logs, posts []string) {
	t.mu.Lock()
	defer t.mu.Unlock()
	return append([]string(nil), t.events...), append([]string(nil), t.logs...), append([]string(nil), t.posts...)
}

func newCharacterizationAPIServer(t *testing.T, auctions []threef.AuctionDto, trace *characterizationTrace) *httptest.Server {
	t.Helper()
...
type characterizationExecutionHarness struct {
	solver   *Solver
	strategy *characterizationStrategy
	signer   *characterizationRecordingSigner
	trace    *characterizationTrace
	stop     func()
}

func newCharacterizationExecutionHarness(
	t *testing.T,
	...
	postErr map[int]bool,
) *characterizationExecutionHarness {
	t.Helper()
	baseSigner, err := signer.NewFromHexKey(characterizationPrivateKey)
	if err != nil {
		t.Fatalf("NewFromHexKey: %v", err)
	}
	recorder := &characterizationRecordingSigner{Signer: baseSigner, failAt: failSignAt}
	trace := &characterizationTrace{postErr: postErr}
	server := newCharacterizationAPIServer(t, auctions, trace)
	...
	strategy := &characterizationStrategy{output: output}
	if laneReady == nil {
		laneReady = func() bool { return true }
	}
	s := &Solver{
		cfg:       &Config{OfferExpiryBuffer: 2 * time.Hour},
		deps:      solver.Deps{Signer: recorder},
	...
	return &characterizationExecutionHarness{
		solver: s, strategy: strategy, signer: recorder, trace: trace,
		stop: func() { stopRPC(); server.Close() },
	}
}

func characterizationExecution(auctionID int64, request, maker common.Address, principal int64, reason string) strategytypes.OfferExecution {
	return strategytypes.OfferExecution{
		AuctionID: auctionID, Request: request, Maker: maker,
		Principal: big.NewInt(principal), ExpectedReturn: big.NewInt(1), Reason: reason,
	}
}

// Test3FR1AuctionIdentityCharacterization freezes raw DTO lookup independently of eligibility.
func Test3FR1AuctionIdentityCharacterization(t *testing.T) {
	t.Parallel()

	collateral := "0x3333333333333333333333333333333333333333"
	first := characterizationAuctionDTO(7.25, "0x1111111111111111111111111111111111111111", collateral)
	second := characterizationAuctionDTO(7.75, "0x2222222222222222222222222222222222222222", collateral)
...
// Test3FR1ExcludedRawIdentityExecutionCharacterization proves that strategy output can name raw rows
// omitted from its input and that the execution lookup still signs/submits them when later checks allow.
func Test3FR1ExcludedRawIdentityExecutionCharacterization(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			valid := characterizationAuctionDTO(1, "0x1111111111111111111111111111111111111111", collateral)
			valid.SetSolveStartTime("2100-01-01T00:00:00Z")
			excluded := characterizationAuctionDTO(2, "0x9999999999999999999999999999999999999999", collateral)
			excluded.SetSolveStartTime("2100-01-01T00:00:00Z")
			test.mutate(&excluded)
			h := newCharacterizationExecutionHarness(t, []threef.AuctionDto{valid, excluded}, strategytypes.OfferOutput{Offers: []strategytypes.OfferExecution{
				characterizationExecution(2, request, maker, 100, "excluded-raw-row"),
			}}, nil, nil, nil)
			defer h.stop()
			before := time.Now()
			h.solver.discoverAndOffer(t.Context())
			after := time.Now()
			calls, inputs := h.strategy.snapshot()
...
// Test3FR1DuplicateExecutionFactsCharacterization freezes last-wins execution facts and proves that
// mismatched and zero strategy Request values are signed as supplied rather than checked against raw DTOs.
func Test3FR1DuplicateExecutionFactsCharacterization(t *testing.T) {
	t.Parallel()
	...
	last := characterizationAuctionDTO(7.75, "0x9999999999999999999999999999999999999999", collateral)
	...
	h := newCharacterizationExecutionHarness(t, []threef.AuctionDto{first, last}, strategytypes.OfferOutput{Offers: []strategytypes.OfferExecution{
		characterizationExecution(7, mismatch, maker, 100, "mismatched-request"),
		characterizationExecution(7, common.Address{}, maker, 100, "zero-request"),
	}}, nil, nil, nil)
	defer h.stop()
...
func Test3FR1DiscoverAndOfferContinuationTrace(t *testing.T) {
	...
	collateral := "0x3333333333333333333333333333333333333333"
	auction := characterizationAuctionDTO(10, "0x1111111111111111111111111111111111111111", collateral)
	...
	request := common.HexToAddress("0x2222222222222222222222222222222222222222")
	nilPrincipal := characterizationExecution(10, request, maker, 1, "nil-principal")
	nilPrincipal.Principal = nil
	h := newCharacterizationExecutionHarness(t, []threef.AuctionDto{auction}, strategytypes.OfferOutput{Offers: []strategytypes.OfferExecution{
		characterizationExecution(999, request, maker, 100, "unknown-auction"),
		characterizationExecution(10, request, unknownMaker, 100, "unknown-maker"),
		nilPrincipal,
		characterizationExecution(10, request, maker, 100, "sign-failure"),
		characterizationExecution(10, request, maker, 100, "submit-failure"),
		characterizationExecution(10, request, maker, 100, "success-after-errors"),
...
func Test3FR1DiscoverAndOfferLaneStopTrace(t *testing.T) {
	...
	auction := characterizationAuctionDTO(20, "0x1111111111111111111111111111111111111111", "0x3333333333333333333333333333333333333333")
	auction.SetSolveStartTime("2100-01-01T00:00:00Z")
	maker := common.HexToAddress("0x00000000000000000000000000000000000000A1")
	request := common.HexToAddress("0x2222222222222222222222222222222222222222")
	h := newCharacterizationExecutionHarness(t, []threef.AuctionDto{auction}, strategytypes.OfferOutput{Offers: []strategytypes.OfferExecution{
		characterizationExecution(20, request, maker, 100, "before-pause"),
		characterizationExecution(20, request, maker, 100, "after-pause"),
		characterizationExecution(20, request, maker, 100, "never-built"),
	}}, laneReady, nil, nil)
	defer h.stop()
...
var _ strategytypes.Strategy = (*characterizationStrategy)(nil)
