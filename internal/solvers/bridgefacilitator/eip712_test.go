...
func TestOfferDigest_MatchesApitypes(t *testing.T) {
	...
	testcheck.NoError(t, err, "hash domain: %v")
	msgHash, err := typed.HashStruct("Offer", typed.Message)
	testcheck.NoError(t, err, "hash message: %v")
...
func TestAPIKeyDigest_MatchesLiveAcceptedSignature(t *testing.T) {
	...
	testcheck.NoError(t, err, "sign: %v")
...
func TestGetOffersDigest_Golden(t *testing.T) {
	...
	// GOLDEN: pinned from TestGruntAPIDigestsMatchApitypes/GetOffers cross-check (chainId 1).
...
// Both grunt-api flows have no verifyingContract (name/version/chainId only).
// Cross-check their schemas against go-ethereum's independent EIP-712 implementation.
func TestGruntAPIDigestsMatchApitypes(t *testing.T) {
	maker := common.HexToAddress("0x42")
	deadline, chainID := big.NewInt(4102444800), big.NewInt(apiKeyDomainChainID)
	for _, tc := range []struct {
		...
		{"CancelOffer", CancelOfferDigest(maker, big.NewInt(192), deadline, chainID),
			[]apitypes.Type{{Name: "maker", Type: "address"}, {Name: "offerId", Type: "uint256"}, {Name: "deadline", Type: "uint256"}},
			apitypes.TypedDataMessage{"maker": maker.Hex(), "offerId": "192", "deadline": deadline.String()}},
		{"GetOffers", GetOffersDigest(maker, deadline, chainID),
			[]apitypes.Type{{Name: "maker", Type: "address"}, {Name: "deadline", Type: "uint256"}},
			apitypes.TypedDataMessage{"maker": maker.Hex(), "deadline": deadline.String()}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			typed := apitypes.TypedData{
				...
				PrimaryType: tc.name,
				Domain:      apitypes.TypedDataDomain{Name: apiKeyDomainName, Version: apiKeyDomainVersion, ChainId: math.NewHexOrDecimal256(apiKeyDomainChainID)},
				Message:     tc.message,
			}
			domainSep, err := typed.HashStruct("EIP712Domain", typed.Domain.Map())
			testcheck.NoError(t, err, "hash domain: %v")
			msgHash, err := typed.HashStruct(tc.name, typed.Message)
			testcheck.NoError(t, err, "hash message: %v")
			want := crypto.Keccak256Hash([]byte{0x19, 0x01}, domainSep, msgHash)
			if tc.got != want {
				t.Fatalf("digest mismatch: manual %s, apitypes %s", tc.got.Hex(), want.Hex())
			}
		})
	}
}
...
func TestGetOffersDigest_MatchesLiveAcceptedSignature(t *testing.T) {
	...
	testcheck.NoError(t, err, "build request: %v")
	...
	testcheck.NoError(t, err, "GET /v1/offer: %v")
