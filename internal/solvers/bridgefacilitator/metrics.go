...
type threeFMetrics struct {
	...
	observedItems    *prometheus.GaugeVec
...
func newThreeFMetrics(reg prometheus.Registerer) (*threeFMetrics, error) {
	m := &threeFMetrics{
		...
		observedItems: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Name: "threef_observed_items",
			Help: "Items in the last complete 3F observation by view.",
		}, []string{"view"}),
	...
	for _, collector := range []prometheus.Collector{
		m.offerSubmissions, m.offerAmounts, m.observedItems, m.lastObservation,
	} {
	...
	for _, view := range []string{threeFStateOffers, threeFStateActiveRequests, threeFStateRedeemable} {
		m.observedItems.WithLabelValues(view)
...
func (s *Solver) observeState(view string, count int) {
	...
	s.metrics.observedItems.WithLabelValues(view).Set(float64(count))
