...
func OfferDigest(offer Offer, domainName, domainVersion string, chainID *big.Int, request common.Address) common.Hash {
	return typedDataDigest(
		domainSeparator(domainName, domainVersion, chainID, request),
		offerStructHash(offer),
	)
}
...
func offerStructHash(o Offer) common.Hash {
	return hashWords(
		...
		o.Maker.Bytes(),
		o.Amount.Bytes(),
		o.ExpectedReturn.Bytes(),
		o.Nonce.Bytes(),
		o.Expiration.Bytes(),
		boolWord(o.UseCallback),
	)
}
...
func typedDataDigest(domain, message common.Hash) common.Hash {
	return crypto.Keccak256Hash([]byte{0x19, 0x01}, domain.Bytes(), message.Bytes())
}
...
func GetOffersDigest(maker common.Address, deadline, chainID *big.Int) common.Hash {
	message := hashWords(getOffersTypeHash.Bytes(), maker.Bytes(), deadline.Bytes())
	return typedDataDigest(gruntAPIDomainSeparator(chainID), message)
}
