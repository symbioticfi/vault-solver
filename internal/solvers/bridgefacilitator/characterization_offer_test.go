...
import (
	...
	solverframework "github.com/symbioticfi/vault-solver/internal/solver"
	strategytypes "github.com/symbioticfi/vault-solver/internal/solvers/bridgefacilitator/strategies/types"
)

const characterizationPrivateKey = "ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"

type characterizationRecordingSigner struct {
	signer.Signer
...
func (s *characterizationRecordingSigner) SignHash(hash common.Hash) ([]byte, error) {
...
// Test3FR1YieldCharacterization is the immutable 3F-R1 solver-side yield backstop baseline.
func Test3FR1YieldCharacterization(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "floor exact but partial unsafe", expectedReturn: big.NewInt(1), principal: big.NewInt(1000), minimum: big.NewInt(100), minimumYieldPpm: big.NewInt(190), maxRateBps: 250, want: "yield is unsafe for partial consumption at minAssetsPerRequest 100"},
		{name: "positive margin but partial unsafe", expectedReturn: big.NewInt(2), principal: big.NewInt(1000), minimum: big.NewInt(100), minimumYieldPpm: big.NewInt(190), maxRateBps: 250, want: "yield is unsafe for partial consumption at minAssetsPerRequest 100"},
		{name: "zero minimum uses one unit and is unsafe", expectedReturn: big.NewInt(2), principal: big.NewInt(1000), minimum: big.NewInt(0), minimumYieldPpm: big.NewInt(190), maxRateBps: 250, want: "yield is unsafe for partial consumption at minAssetsPerRequest 1"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			err := strategytypes.ValidateYield(test.expectedReturn, test.principal, test.minimum, test.minimumYieldPpm, test.maxRateBps)
			if test.want == "" {
				if err != nil {
...
// Test3FR1SignedOfferCharacterization freezes nullable-domain handling, version fallback, expiration,
// nonce consumption on signer failure, the selected digest, and the resulting complete DTO.
func Test3FR1SignedOfferCharacterization(t *testing.T) { //nolint:cyclop,maintidx // One immutable table spans domain, nonce, signature, DTO, and expiration behavior.
	t.Parallel()
	...
	execution := strategytypes.OfferExecution{
	...
	valid := func() threef.AuctionDto {
		dto := characterizationAuctionDTO(77, "0x9999999999999999999999999999999999999999", "0x3333333333333333333333333333333333333333")
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			dto := valid()
			test.mutate(&dto)
			s := &Solver{cfg: &Config{OfferExpiryBuffer: 2 * time.Hour}, deps: solverframework.Deps{Signer: baseSigner}}
	...
	for _, test := range versionCases {
		...
		recorder := &characterizationRecordingSigner{Signer: baseSigner}
		s := &Solver{cfg: &Config{OfferExpiryBuffer: 2 * time.Hour}, deps: solverframework.Deps{Signer: recorder}, nonceSeq: 40}
	...
	customDTO := valid()
	recorder := &characterizationRecordingSigner{Signer: baseSigner, failAt: map[int]error{1: stderrors.New("injected signing failure")}}
	s := &Solver{cfg: &Config{OfferExpiryBuffer: 2 * time.Hour}, deps: solverframework.Deps{Signer: recorder}, nonceSeq: 500}
	...
	customRecorder := &characterizationRecordingSigner{Signer: baseSigner}
	customSolver := &Solver{cfg: &Config{OfferExpiryBuffer: 2 * time.Hour}, deps: solverframework.Deps{Signer: customRecorder}, nonceSeq: 501}
