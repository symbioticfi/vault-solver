...
//
//	go test -tags live -run TestLiveAPIMonitorSnapshotAndCandidates -v ./internal/solvers/redstoneoev/
func TestLiveAPIMonitorSnapshotAndCandidates(t *testing.T) {
	...
	apiSnap := mon.apiMarketSnapshot(apiMarkets, loan, []common.Address{coll}, true)
	...
	auction := AuctionMessage{Payload: AuctionPayload{Prices: map[string]string{oracle.Hex(): price.String()}}}
	cands := mon.candidates(auction, apiSnap.blockTime)
	if len(cands) == 0 {
