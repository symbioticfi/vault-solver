...
func (a *morphoClient) DiscoverMarketData(ctx context.Context, chainID int64, loan, collateral []common.Address) ([]morphoMarket, error) {
	...
	data, err := morphographql.MorphoDiscoverMarkets(ctx, a.gql, lowerAddrs(loan), lowerAddrs(collateral), []int{int(chainID)}, maxDiscoverMarkets)
	if err != nil {
		return nil, err
	}
...
func (a *morphoClient) PositionsByMarket(ctx context.Context, marketIDs []common.Hash, first int, maxHF *float64) ([]morphoPosition, error) {
	...
	for _, id := range marketIDs {
		ids = append(ids, strings.ToLower(id.Hex()))
	}
...
// lowerAddrs maps addresses to their lowercase 0x hex form (the Morpho API matches addresses lowercased).
func lowerAddrs(addrs []common.Address) []string {
