...
// Snapshot captures one immutable planner input at the auction price. RedStone's pushed positions are ignored.
func (m *apiMonitor) Snapshot(
	auction decision.AuctionSnapshot,
	now time.Time,
	adapter decision.AdapterSnapshot,
) decision.MarketFacts {
	snap := m.snapshot()
	return marketFacts(snap, candidatesFromAuctionWithAdapter(m.log, snap, auction, clampTsAt(auction.Timestamp, now), adapter))
}
...
func adapterMarketScope(adapter decision.AdapterSnapshot) (common.Address, []common.Address, bool) {
...
func (m *apiMonitor) apiMarketSnapshot(apiMarkets []morphoMarket, loan common.Address, redeemable []common.Address) apiMarketSnapshot {
	...
	for _, apiMarket := range apiMarkets {
		...
		_, collateralAllowed := redeem[view.info.Params.CollateralToken]
		if !ok || view.info.Params.LoanToken != loan || !collateralAllowed {
			continue
		}
...
func snapshotFreshForAuction(facts decision.MarketFacts, auction decision.AuctionSnapshot) string {
	if !facts.HasPositions {
		return ""
	}
	if facts.Block == 0 || facts.BlockTime == 0 {
	...
	if uint64(auctionTs) > facts.BlockTime+uint64(snapshotMaxAuctionLag/time.Second) {
		return skipStaleEpoch
	}
	return ""
}
