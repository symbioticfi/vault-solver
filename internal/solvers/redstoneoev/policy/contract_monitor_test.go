...
func monitorAdapterSnapshot(collateral common.Address) decision.AdapterSnapshot {
...
func TestMonitorCandidatePricePolicy(t *testing.T) {
	...
	auction := decision.AuctionSnapshot{
		Prices: []decision.AuctionPrice{{Oracle: oracle, Price: framePx}},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got := test.mon.Snapshot(auction, time.Unix(int64(snap.markets[id].State.LastUpdate), 0), adapter).Candidates
...
func TestTestMonitorCandidatePriceDoesNotRequireAuctionFrame(t *testing.T) {
	...
	got := mon.Snapshot(
		decision.AuctionSnapshot{}, time.Unix(int64(snap.markets[id].State.LastUpdate), 0), monitorAdapterSnapshot(collateral),
	).Candidates
...
func TestAdapterMarketScopeUsesSolverSnapshot(t *testing.T) {
	...
	gotLoan, gotRedeemable, ok := adapterMarketScope(decision.AdapterSnapshot{
		Loan: loan,
		Redeemable: []decision.RedeemableSnapshot{
...
func TestCandidateRequiresAuctionPriceForMarketOracle(t *testing.T) {
	...
	auction := decision.AuctionSnapshot{Prices: []decision.AuctionPrice{
...
func TestSnapshotFreshForAuction(t *testing.T) {
	...
	auction := decision.AuctionSnapshot{Timestamp: auctionAt}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := snapshotFreshForAuction(marketFacts(tc.snap, nil), auction); got != tc.want {
