...
func (in QuoteInput) MarshalJSON() ([]byte, error) {
	...
	for _, c := range in.Candidates {
		candidates = append(candidates, quoteCandidateJSON{
			...
			MaxAssets:     webhook.FormatDecimal(c.MaxAmountOut), MaxRate: webhook.FormatDecimal(c.Rate), DiscountID: c.DiscountID,
		})
	}
	return json.Marshal(quoteInputJSON{
		RequestID: in.RequestID, QuoteID: in.QuoteID, ChainID: in.ChainID,
		Executor: in.Executor, TokenIn: in.TokenIn, TokenOut: in.TokenOut, AmountIn: webhook.FormatDecimal(in.AmountIn),
		RequiredAmountOut: webhook.FormatDecimal(in.RequiredAmountOut), RequireSingleRoute: in.RequireSingleRoute,
...
func (out *QuoteOutput) UnmarshalJSON(b []byte) error {
	...
	quoted, err := webhook.ParseOptionalDecimal(raw.QuotedAmountOut, "quotedAmountOut")
	if err != nil {
		return err
	}
	legs := make([]QuoteLeg, 0, len(raw.Legs))
	for i, l := range raw.Legs {
		amountIn, err := webhook.ParseOptionalDecimal(l.AmountIn, "legs.amountIn")
		if err != nil {
			return errors.Errorf("leg %d: %w", i, err)
		}
		amountOut, err := webhook.ParseOptionalDecimal(l.AmountOut, "legs.amountOut")
		if err != nil {
			return errors.Errorf("leg %d: %w", i, err)
		}
