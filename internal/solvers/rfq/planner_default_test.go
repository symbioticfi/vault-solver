...
func baseInput(candidates ...liquidlane.QuoteCandidate) QuoteInput {
	return QuoteInput{
...
func TestStrategyQuotesNormalizedLiquidLaneCandidates(t *testing.T) {
	...
	got, err := newDefaultPlanner().DecideQuote(t.Context(), baseInput(candidate))
	if err != nil {
		t.Fatalf("DecideQuote: %v", err)
	}
	if got.Decision != DecisionQuote || got.QuotedAmountOut.Cmp(big.NewInt(200)) != 0 ||
...
func TestStrategyAggregatesRoutesButHonorsSingleRoute(t *testing.T) {
	...
	got, err := newDefaultPlanner().DecideQuote(t.Context(), baseInput(first, second))
	if err != nil || got.Decision != DecisionQuote || len(got.Legs) != 2 ||
	...
	got, err = newDefaultPlanner().DecideQuote(t.Context(), input)
	if err != nil || got.Decision != DecisionQuote || len(got.Legs) != 1 ||
...
func TestStrategySingleRouteQuotesCappedOutputWhenInputExceedsCapacity(t *testing.T) {
	...
	got, err := newDefaultPlanner().DecideQuote(t.Context(), input)
	if err != nil || got.Decision != DecisionQuote ||
...
func TestStrategyTreatsDirectAndDiscountAsRouteAlternatives(t *testing.T) {
	...
	got, err := newDefaultPlanner().DecideQuote(t.Context(), baseInput(private, direct))
	if err != nil || got.Decision != DecisionQuote || len(got.Legs) != 1 ||
...
func TestBuildFillPlanUsesTypedCandidateWithoutRepricing(t *testing.T) {
	...
	plan, err := newDefaultPlanner().BuildFillPlan(t.Context(), input)
	if err != nil || plan == nil || len(plan.Routes) != 1 {
		t.Fatalf("plan = %+v, err %v", plan, err)
	}
	leg := plan.Routes[0]
	if leg.Adapter != vlt || leg.AmountIn.Cmp(big.NewInt(100)) != 0 ||
		leg.ExpectedAmountOut.Cmp(big.NewInt(100)) != 0 ||
...
func TestBuildFillPlanSingleRouteKeepsCappedQuoteWhenInputExceedsCapacity(t *testing.T) {
	...
	plan, err := newDefaultPlanner().BuildFillPlan(t.Context(), input)
	if err != nil || plan == nil || len(plan.Routes) != 1 ||
		plan.Routes[0].AmountIn.Cmp(big.NewInt(100)) != 0 ||
		plan.Routes[0].ExpectedAmountOut.Cmp(big.NewInt(120)) != 0 {
...
func TestBuildFillPlanRejectsNonCanonicalCandidateID(t *testing.T) {
	candidate := quoteCandidate(vlt, 2, 100, 200, nil)
	candidate.ID = "wrong"
	plan, err := newDefaultPlanner().BuildFillPlan(t.Context(), baseInput(candidate))
	if err == nil || plan != nil {
