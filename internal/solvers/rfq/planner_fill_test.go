...
func TestFillPlanFromQuote(t *testing.T) {
	...
	out := QuoteOutput{
		Decision: DecisionQuote, QuotedAmountOut: big.NewInt(95),
		Legs: []QuoteLeg{{CandidateID: string(candidateID), AmountIn: big.NewInt(100), AmountOut: big.NewInt(95)}},
	}
	...
	if plan == nil || len(plan.Routes) != 1 ||
		plan.Routes[0].Adapter != adapter || plan.Routes[0].AmountIn.Cmp(input.AmountIn) != 0 ||
		plan.Routes[0].ExpectedAmountOut.Cmp(out.QuotedAmountOut) != 0 {
...
func TestFillPlanFromQuoteRejectsInconsistentDecision(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			out: QuoteOutput{
				Decision: DecisionQuote, QuotedAmountOut: big.NewInt(90),
				Legs: []QuoteLeg{{CandidateID: "unknown", AmountIn: big.NewInt(100), AmountOut: big.NewInt(90)}},
			},
		},
		{
			name: "input sum",
			out: QuoteOutput{
				Decision: DecisionQuote, QuotedAmountOut: big.NewInt(90),
				Legs: []QuoteLeg{{CandidateID: string(candidateID), AmountIn: big.NewInt(99), AmountOut: big.NewInt(90)}},
			},
		},
		{
			name: "output capacity",
			out: QuoteOutput{
				Decision: DecisionQuote, QuotedAmountOut: big.NewInt(101),
				Legs: []QuoteLeg{{CandidateID: string(candidateID), AmountIn: big.NewInt(100), AmountOut: big.NewInt(101)}},
			},
		},
	}
...
func TestFillPlanFromQuoteRejectsRepeatedRoute(t *testing.T) {
	...
	out := QuoteOutput{Decision: DecisionQuote, Legs: legs, QuotedAmountOut: big.NewInt(100)}
	if _, err := FillPlanFromQuote(input, out); err == nil {
		t.Fatal("FillPlanFromQuote returned nil error")
	}
}
