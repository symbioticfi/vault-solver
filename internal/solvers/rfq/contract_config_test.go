...
reactor: "0x0000000000000000000000000000000000000020"
`
...
func TestParseConfig_Defaults(t *testing.T) {
	...
	if cfg.SolverMode != liquidlane.SolverModeExternal {
		t.Fatalf("solverMode = %q, want %q", cfg.SolverMode, liquidlane.SolverModeExternal)
	}
	...
	if cfg.Reactor != common.HexToAddress("0x0000000000000000000000000000000000000020") {
		t.Fatalf("reactor = %s", cfg.Reactor.Hex())
	}
}
...
func TestParseConfig_SolverMode(t *testing.T) {
	...
	cases := map[string]struct {
		yaml          string
		wantMode      liquidlane.SolverMode
		wantRestrict  bool
		wantDiscounts bool
		wantErr       bool
	}{
		"external + adapters":         {yaml: "solverMode: external" + a, wantMode: liquidlane.SolverModeExternal, wantRestrict: true},
		"external, no adapters":       {yaml: "solverMode: external", wantErr: true},
		"internal + adapters":         {yaml: "solverMode: internal" + a, wantMode: liquidlane.SolverModeInternal, wantDiscounts: true},
		"internal, no adapters":       {yaml: "solverMode: internal", wantMode: liquidlane.SolverModeInternal, wantDiscounts: true},
		"default + adapters":          {yaml: a, wantMode: liquidlane.SolverModeExternal, wantRestrict: true},
