...
`,
		...
		},
		{
			...
			wantName:          "missing",
			wantValidationErr: `strategy: unknown LI.FI strategy "missing" (registered: [default webhook])`,
			wantSelectionErr:  `unknown LI.FI strategy "missing" (registered: [default webhook])`,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			node := lifiSelectionNode(t, lifiSelectionBase+test.strategy)
	...
	t.Run("root rejects unknown key", func(t *testing.T) {
		if _, err := parseConfig(lifiSelectionNode(t, lifiSelectionBase+"unexpected: true\n")); err == nil {
			t.Fatal("parseConfig accepted an unknown root key")
		}
	})
}

func lifiSelectionNode(t *testing.T, raw string) yaml.Node {
	t.Helper()
...
func assertLIFIStrategyType(t *testing.T, strategy types.Strategy, want string) {
	t.Helper()
	switch want {
	case "default":
