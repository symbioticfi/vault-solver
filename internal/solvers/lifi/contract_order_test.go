...
func TestOrderInboxKeyUsesOrderPayloadInsteadOfMetadata(t *testing.T) {
	...
	if orderKey(first) != orderKey(replay) {
...
func TestParseSubmittedOrderRejectsNonOnChainOrderType(t *testing.T) {
	...
	_, err = parseSubmittedOrder(raw, cfg, 11155111)
	if err == nil || !errors.Is(err, errOrderUnsupported) {
		t.Fatalf("err = %v", err)
	}
	var logs []string
	solver := &Solver{cfg: cfg, chainID: 11155111, log: funcr.NewJSON(func(line string) {
		logs = append(logs, line)
	}, funcr.Options{Verbosity: 1})}
	if solver.parseOrderMessage(orderMessage{Event: orderSubmitEvent, Data: raw}) != nil {
		t.Fatal("unsupported order was accepted")
	}
	logged := strings.Join(logs, "\n")
	if strings.Contains(logged, `"error"`) || !strings.Contains(logged, "ignored unsupported order") {
		t.Fatalf("unexpected log level: %s", logged)
	}
