...
func (s *defaultPlanner) buildQuoteRanges(
	candidates []liquidlane.QuoteCandidate,
	maxRoutes int,
	pricing liquidplanning.GasPricing,
) ([]QuoteRange, map[liquidlane.CandidateID]liquidlane.QuoteCandidate, error) {
	candidates = liquidplanning.BestRouteCandidates(candidates, maxRoutes)
	if len(candidates) == 0 {
		return nil, nil, nil
	}
	...
	breakpoints := quoteBreakpoints(maximum, s.policy.MinAmount, s.rangeCount)
	used := make(map[liquidlane.CandidateID]liquidlane.QuoteCandidate)
	ranges := make([]QuoteRange, 0, len(breakpoints))
...
func (s *defaultPlanner) firstSafeRangeInput(
	candidates []liquidlane.QuoteCandidate,
	lower *big.Int,
	upper *big.Int,
	maxGasCost *big.Int,
...
func (s *defaultPlanner) priceQuoteRange(
	candidates []liquidlane.QuoteCandidate,
	maxRoutes int,
	lower *big.Int,
	upper *big.Int,
	...
	quoteAt := func(amount *big.Int) (*liquidplanning.QuoteSolution, error) {
		return liquidplanning.SolveQuote(liquidplanning.QuoteTask{
			ExactInput:      amount,
			Candidates:      candidates,
			MaxRoutes:       maxRoutes,
			MinInput:        s.policy.MinAmount,
			OutputBufferBps: 2 * s.policy.PriceBufferBps,
			InputPolicy:     liquidplanning.RejectUncoveredInput,
			GasPricing:      &pricing,
		})
	}
