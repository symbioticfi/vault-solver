...
type readyTransactionLane struct{}

func (readyTransactionLane) LaneReady() bool { return true }
func (readyTransactionLane) SubscribeLaneState() (<-chan struct{}, func()) {
	return make(chan struct{}), func() {}
}

func alwaysReadyTransactionLane() transactionLaneState { return readyTransactionLane{} }

func TestQuoteExpiryBoundsUsesPublishedExpiries(t *testing.T) {
	earliest, latest := quoteExpiryBounds([]Quote{{Expiry: 30}, {Expiry: 10}, {Expiry: 20}})
...
func (*recordingQuoteStrategy) DecideFill(context.Context, FillInput) (FillDecision, error) {
	return FillDecision{}, nil
}
...
func TestRefreshQuotesWithoutGasAccounting(t *testing.T) {
	...
	feeReads := 0
	solver := &Solver{
		cfg: cfg, reader: fakeLifiReader{}, planner: strategy, log: logr.Discard(),
		capacity:    new(capacity.Book),
...
func (f *fakeQuoteSubmitter) submitQuotes(_ context.Context, quotes []Quote) error {
	copyOfQuotes := append([]Quote(nil), quotes...)
...
func TestQuoteStatePublishesAndReplacesChangedTopology(t *testing.T) {
	...
	removed, err := state.reconcile(context.Background(), submitter, []Quote{first}, now)
	if err != nil {
	...
	if _, err := state.reconcile(context.Background(), submitter, []Quote{second}, now); err != nil {
	...
	removed, err = state.reconcile(context.Background(), submitter, []Quote{changed}, now)
	if err != nil {
...
func TestQuoteStateSkipsUnchangedPairUntilRenewalWindow(t *testing.T) {
	...
	if _, err := state.reconcile(context.Background(), submitter, []Quote{quote}, now); err != nil {
		t.Fatalf("publish: %v", err)
	}
	refreshed := testStandingQuote(routeItem, 1_000)
	refreshed.Expiry += 60
	if _, err := state.reconcile(context.Background(), submitter, []Quote{refreshed}, now.Add(30*time.Second)); err != nil {
		t.Fatalf("unchanged: %v", err)
	}
	if len(submitter.calls) != 1 {
		t.Fatalf("unchanged pair was republished: calls=%d", len(submitter.calls))
	}
	if _, err := state.reconcile(context.Background(), submitter, []Quote{refreshed}, now.Add(90*time.Second)); err != nil {
...
func TestQuoteStateRemovesPairWhenStrategyStopsQuoting(t *testing.T) {
	...
	submitter := &fakeQuoteSubmitter{}
	now := time.Unix(1_800_000_000, 0)
	if _, err := state.reconcile(context.Background(), submitter, []Quote{testStandingQuote(routeItem, 1_000)}, now); err != nil {
		t.Fatalf("publish: %v", err)
	}
	submitter.calls = nil
...
func TestQuoteStateRetriesExpireAfterPartialSubmitAcknowledgement(t *testing.T) {
	...
	if _, err := state.reconcile(context.Background(), client, []Quote{quote}, now); err != nil {
		t.Fatalf("publish: %v", err)
	}
...
func testStandingQuote(route route, maxAmount int64) Quote {
	return Quote{
