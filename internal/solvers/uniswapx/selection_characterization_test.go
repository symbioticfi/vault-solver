...
`,
		...
		},
		{
			...
			wantName:          "missing",
			wantValidationErr: `strategy: unknown UniswapX strategy "missing" (registered: [default webhook])`,
			wantSelectionErr:  `unknown UniswapX strategy "missing" (registered: [default webhook])`,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			node := uniswapXSelectionNode(t, raw)
	...
	t.Run("root rejects unknown key", func(t *testing.T) {
		if _, err := parseConfig(uniswapXSelectionNode(t, validUniswapXConfig+"unexpected: true\n")); err == nil {
			t.Fatal("parseConfig accepted an unknown root key")
		}
	})
}

func uniswapXSelectionNode(t *testing.T, raw string) yaml.Node {
	t.Helper()
...
func assertUniswapXStrategyType(t *testing.T, strategy types.Strategy, want string) {
	t.Helper()
	switch want {
	case "default":
