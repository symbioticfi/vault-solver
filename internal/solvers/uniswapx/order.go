...
func mustV2CosignerDataArguments() abi.Arguments {
	order := v2OrderArguments[0].Type
	for index, field := range order.TupleRawNames {
		if field == "cosignerData" {
			return abi.Arguments{{Type: *order.TupleElems[index]}}
		}
	}
	panic("V2 order schema has no cosignerData")
}
...
type decodedV2Entry struct {
	order   v2Order
	encoded []byte
	hash    common.Hash
}

// Decode and bind the backend identifier once, before either obligation tracking
// or fill resolution consumes the order.
func decodeV2Entry(entry orderEntry, chainID int64) (decodedV2Entry, error) {
	if entry.Type != orderTypeDutchV2 {
		return decodedV2Entry{}, errors.Errorf("unsupported order type %q", entry.Type)
	}
	if entry.ChainID != chainID {
		return decodedV2Entry{}, errors.Errorf("order chain id %d does not match %d", entry.ChainID, chainID)
	}
	encoded, err := hexutil.Decode(entry.EncodedOrder)
	if err != nil {
		return decodedV2Entry{}, errors.Errorf("encodedOrder: %w", err)
	}
	values, err := v2OrderArguments.Unpack(encoded)
	if err != nil {
		return decodedV2Entry{}, errors.Errorf("decode V2 order: %w", err)
	}
	if len(values) != 1 {
		return decodedV2Entry{}, errors.New("decode V2 order: expected one tuple")
	}
	order := *abi.ConvertType(values[0], new(v2Order)).(*v2Order)
	hash, err := v2OrderHash(order)
	if err != nil {
		return decodedV2Entry{}, err
	}
	return decodedV2Entry{order: order, encoded: encoded, hash: hash}, nil
}

func (d decodedV2Entry) verifyHash(raw string) error {
	advertised, err := parseHash(raw)
	if err != nil || advertised != d.hash {
		return errors.New("orderHash does not match encoded order")
	}
	return nil
}

func exclusiveObligationFromEntry(entry orderEntry, cfg *Config, chainID int64) (exclusiveObligation, error) {
	decoded, err := decodeV2Entry(entry, chainID)
	if err != nil {
		return exclusiveObligation{}, err
	}
	if err := decoded.verifyHash(entry.OrderHash); err != nil {
		return exclusiveObligation{}, err
	}
	data := decoded.order.CosignerData
	if data.ExclusiveFiller != cfg.Executor {
		return exclusiveObligation{}, errors.Errorf("%w: got %s", errDifferentExclusiveFiller, data.ExclusiveFiller.Hex())
	}
	deadline, ok := uint64Value(data.DecayStartTime)
	if !ok || deadline == 0 || deadline > math.MaxInt64 {
		return exclusiveObligation{}, errors.New("invalid exclusive deadline")
	}
	return exclusiveObligation{hash: decoded.hash, deadline: time.Unix(int64(deadline), 0)}, nil
}
...
func parseAndResolveOrder(
	...
	if entry.OrderStatus != orderStatusOpen {
		return nil, errors.Errorf("unsupported order status %q", entry.OrderStatus)
	}
	decoded, err := decodeV2Entry(entry, chainID)
	if err != nil {
		return nil, err
	...
	if err := validateCosignature(order, decoded.hash); err != nil {
		return nil, err
	}
	...
	return &resolvedOrder{
		Encoded: decoded.encoded, Signature: signature, Hash: decoded.hash, QuoteID: entry.QuoteID,
...
func v2OrderHash(order v2Order) (common.Hash, error) {
	info, err := hashV2Fields(v2HashArguments.info, v2OrderInfoType, order.Info.Reactor, order.Info.Swapper,
		order.Info.Nonce, order.Info.Deadline, order.Info.AdditionalValidationContract, crypto.Keccak256Hash(order.Info.AdditionalValidationData))
	if err != nil {
		return common.Hash{}, errors.Errorf("hash order info: %w", err)
	}
	outputHashes := make([]byte, len(order.BaseOutputs)*common.HashLength)
	for index, output := range order.BaseOutputs {
		hash, err := hashV2Fields(v2HashArguments.output, v2OutputType, output.Token, output.StartAmount, output.EndAmount, output.Recipient)
		if err != nil {
			return common.Hash{}, errors.Errorf("hash order output %d: %w", index, err)
		}
		copy(outputHashes[index*common.HashLength:], hash[:])
	}
	hash, err := hashV2Fields(v2HashArguments.witness, v2WitnessType, info, order.Cosigner,
		order.BaseInput.Token, order.BaseInput.StartAmount, order.BaseInput.EndAmount, crypto.Keccak256Hash(outputHashes))
	if err != nil {
...
func hashV2Fields(layout abi.Arguments, typeDeclaration string, fields ...any) (common.Hash, error) {
	encoded, err := layout.Pack(append([]any{crypto.Keccak256Hash([]byte(typeDeclaration))}, fields...)...)
	if err != nil {
