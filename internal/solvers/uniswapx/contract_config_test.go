...
func TestParseConfigDefaultsAndSources(t *testing.T) {
	...
	if cfg.QuoteServer.ListenAddress != defaultListenAddress ||
		...
		cfg.SolverMode != liquidlane.SolverModeExternal ||
		cfg.usesDiscounts() {
...
`,
	...
	tests := map[string]struct {
		...
		"default external": {
			base: validUniswapXConfig, wantMode: liquidlane.SolverModeExternal, wantRestrict: true, wantQuote: true,
		},
		...
		"explicit external": {
			...
			wantMode: liquidlane.SolverModeExternal, wantRestrict: true, wantQuote: true,
		},
		...
		"internal with adapters": {
			...
			wantMode: liquidlane.SolverModeInternal, wantDiscounts: true, wantQuote: true,
		},
		"internal without adapters": {
			base: withoutAdapters, suffix: "solverMode: internal\n" + discounts,
			wantMode: liquidlane.SolverModeInternal, wantDiscounts: true,
		},
		"invalid": {
...
func TestParseConfigRejectsRemovedFields(t *testing.T) {
	tests := map[string]string{
		"legacy limit source": strings.Replace(validUniswapXConfig, "publicV2: true", `publicV2: true
    limit:
      reactor: "0x8888888888888888888888888888888888888888"
      executor: "0x2222222222222222222222222222222222222222"`, 1),
		"unknown field": validUniswapXConfig + "unknown: true\n",
		"legacy cosigner pin": strings.Replace(
			validUniswapXConfig,
			"executor: \"0x2222222222222222222222222222222222222222\"",
			"executor: \"0x2222222222222222222222222222222222222222\"\n"+
	...
	for name, raw := range tests {
		t.Run(name, func(t *testing.T) {
			_, err := parseConfig(uniswapXConfigNode(t, raw))
			if err == nil || name == "legacy limit source" && !strings.Contains(err.Error(), "field limit not found") {
				t.Fatalf("err = %v", err)
			}
		})
	}
