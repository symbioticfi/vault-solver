...
func TestDecideQuoteRequiresOneRequestedAmountAndFreshState(t *testing.T) {
	strategy, err := newDefaultPlanner(defaultPlannerConfig{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecideQuoteReturnsOneExactInputAmountWithBuffer(t *testing.T) {
	strategy, err := newDefaultPlanner(defaultPlannerConfig{PriceBufferBps: 100})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecideQuoteUsesCurrentCapacityAndReservations(t *testing.T) {
	strategy, err := newDefaultPlanner(defaultPlannerConfig{InventoryReserveBps: 1_000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecideFillBuildsCurrentMultiRoutePlan(t *testing.T) {
	...
	plan, err := strategy.DecideFill(context.Background(), FillInput{
...
func TestDecideFillHonorsPendingReservationAndDeadline(t *testing.T) {
	strategy, err := newDefaultPlanner(defaultPlannerConfig{ExecutionDeadlineBuffer: "30s"})
	if err != nil {
		t.Fatal(err)
	}
	tokenIn, tokenOut := testPair()
...
func directQuoteInput(amountIn, maxAssets int64, rate *big.Int) QuoteInput {
	tokenIn, tokenOut := testPair()
	now := time.Unix(1_800_000_000, 0)
	return QuoteInput{
