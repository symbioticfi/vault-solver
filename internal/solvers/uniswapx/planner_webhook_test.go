...
func TestWebhookStrategyDelegatesOneQuoteAndCurrentFill(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case decideQuoteRoute:
			_ = json.NewEncoder(w).Encode(Quote{AmountIn: big.NewInt(100), AmountOut: big.NewInt(90)})
	...
	quote, err := strategy.DecideQuote(t.Context(), QuoteInput{
...
func TestDecideFillNullResponseDeclines(t *testing.T) {
	...
	plan, err := newWebhookTestStrategy(t, server.URL).DecideFill(t.Context(), FillInput{})
	if err != nil {
...
func newWebhookTestStrategy(t *testing.T, url string) *webhookPlanner {
	t.Helper()
	client, err := webhook.NewClient(webhook.Config{URL: url, Timeout: time.Second})
	if err != nil {
		t.Fatal(err)
	}
	return &webhookPlanner{client: client}
}
