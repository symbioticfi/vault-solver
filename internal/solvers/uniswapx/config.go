...
func parseConfig(node yaml.Node) (*Config, error) {
	...
	adapters, err := parse.NonZeroAddresses(raw.Adapters, "adapters")
	if err != nil {
		return nil, err
	}
	...
	if solverMode == liquidlane.SolverModeExternal && len(adapters) == 0 {
	...
	if solverMode == liquidlane.SolverModeInternal && raw.Discounts == nil {
		return nil, errors.New("discounts is required in internal solverMode")
	}
	if solverMode == liquidlane.SolverModeExternal && raw.Discounts != nil {
