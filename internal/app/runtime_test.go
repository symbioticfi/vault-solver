...
func TestMonitorTransactionDrainForcesStopAfterTimeout(t *testing.T) {
	...
	close(shutdown)
	testcheck.ReceiveWithin(t, stopped, time.Second, "transaction drain did not force stop after timeout")
	testcheck.ReceiveWithin(t, done, time.Second, "transaction drain monitor did not return after timeout")
}
...
func TestMonitorTransactionDrainStopsWhenSolversFinish(t *testing.T) {
	...
	close(shutdown)
	close(solversDone)
	testcheck.ReceiveWithin(t, done, time.Second, "transaction drain monitor did not stop with solvers")
	select {
	case <-stopped:
