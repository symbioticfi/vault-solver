...
type runtimeTransactionManager = TransactionLane

func runSolverLifecycle(
	ctx context.Context,
	cfg *config.Config,
	manager runtimeTransactionManager,
	solvers []Integration,
	requiresTransactionManager bool,
	setReady func(bool),
	log logr.Logger,
) error {
	return Run(
		ctx,
		RunConfig{
			ConfigSource: "contract.yaml", RequiresTransactionLane: requiresTransactionManager,
			ValidateTxConfig:    cfg.ValidateTxManager,
...
func newContractManager(trace *lifecycleTrace) *contractManager {
...
func (m *contractManager) ValidateFeeHeadroom() error {
	m.validateCalls.Add(1)
	m.trace.add("manager validate")
	return m.validateErr
}

func (m *contractManager) Initialize(context.Context) error {
	m.initCalls.Add(1)
	m.trace.add("manager initialize")
	return m.initializeErr
}

func (m *contractManager) Start(ctx context.Context) {
	m.startCalls.Add(1)
...
func (m *contractManager) SubscribeLaneState() (<-chan struct{}, func()) {
...
func (m *contractManager) LaneReady() bool {
	m.laneCalls.Add(1)
...
func newContractHealth(trace *lifecycleTrace) *contractHealth {
	return &contractHealth{trace: trace, updates: make(chan bool, 16)}
}

func (h *contractHealth) SetReady(ready bool) {
	h.calls.Add(1)
	h.trace.add(fmt.Sprintf("readiness %t", ready))
	h.updates <- ready
}

type contractSolver struct {
	name string
	run  func(context.Context) error
}

func (s contractSolver) Name() string { return s.name }
func (s contractSolver) Run(ctx context.Context) error {
	return s.run(ctx)
}
...
func testSolverLifecycleSelection(t *testing.T, solverNames []string, requiresManager bool) {
	...
	health := newContractHealth(trace)
	manager := newContractManager(trace)
	solvers := make([]Integration, 0, len(solverNames))
	solverStarted := make(chan struct{}, len(solverNames))
	for _, name := range solverNames {
		solvers = append(solvers, contractSolver{name: name, run: func(ctx context.Context) error {
			if requiresManager {
				<-manager.started
			}
	...
	go func() {
		returned <- runSolverLifecycle(
			ctx, lifecycleConfig(), runtimeManager,
...
func TestSolverLifecycleShutdownTrace(t *testing.T) {
	...
	accepted := contractSolver{name: "accepted", run: func(ctx context.Context) error {
		<-manager.started
	...
	fatal := contractSolver{name: "fatal", run: func(context.Context) error {
		<-acceptedStarted
	...
	go func() {
		err := runSolverLifecycle(
			t.Context(), lifecycleConfig(), manager,
			[]Integration{accepted, fatal}, true, health.SetReady, logr.Discard(),
		)
...
func TestSolverLifecycleTimeoutStopsManagerWithoutAbandoningSolverJoin(t *testing.T) {
	...
	blocking := contractSolver{name: "accepted", run: func(ctx context.Context) error {
		<-manager.started
		close(solverStarted)
		<-ctx.Done()
		trace.add("accepted solver intake stopped")
	...
	go func() {
		returned <- runSolverLifecycle(
			ctx, timeoutLifecycleConfig(), manager,
			[]Integration{blocking}, true, health.SetReady, log,
		)
	}()
	expectReadyState(t, health.updates, true)
	waitForSignal(t, solverStarted, "solver start")
...
func TestSolverLifecycleManagerStartupFailuresDoNotStartRuntime(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:        `invalid config "contract.yaml": txManager: invalid fee headroom`,
		},
		{
			name:          "initialization",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := runSolverLifecycle(
				t.Context(), lifecycleConfig(), manager,
				[]Integration{contractSolver{name: "must-not-run", run: func(context.Context) error {
					solverCalls.Add(1)
					return nil
