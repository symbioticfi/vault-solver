...
func TestServeUntilCompletesOnCancellation(t *testing.T) {
	...
	srv := NewHTTPServer(addr, NewMetrics("test", "test"))
...
func TestServeUntilCompletesOnBindError(t *testing.T) {
	...
	var logs []string
	log := funcr.NewJSON(func(entry string) { logs = append(logs, entry) }, funcr.Options{})
	srv := NewHTTPServer(listener.Addr().String(), NewMetrics("test", "test"))
	done := make(chan struct{})
	go func() {
		defer close(done)
...
func TestNewMetricsRegistersBuildAndRuntimeCollectors(t *testing.T) {
	...
	metrics.SetSolvers([]string{"rfq-filler"})
	...
	for _, name := range []string{
		"solver_bot_solver_info", "solver_bot_service_ready", "go_goroutines", "process_cpu_seconds_total",
	} {
	...
	if !metrics.Ready() {
		t.Fatal("readiness state was not published")
	}
}
