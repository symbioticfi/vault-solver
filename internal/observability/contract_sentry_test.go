...
func TestNewLoggerCleanupFlushesSentryOnce(t *testing.T) {
	...
	if transport.flushes != 1 {
		t.Fatalf("expected logger cleanup to flush Sentry once, got %d flushes", transport.flushes)
	}
	if transport.lastTimeout != sentryFlushTimeout {
		t.Fatalf("expected Sentry flush timeout %s, got %s", sentryFlushTimeout, transport.lastTimeout)
	}
}
