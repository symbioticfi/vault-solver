...
| Solver-local strategy contract and built-ins | `internal/solvers/bridgefacilitator` |
...
Offer discovery/reconciliation and redemption run as independent coordinator loops. Target refresh publishes
an immutable adapter snapshot for redemption, so a slow or unavailable 3F HTTP API cannot delay finalization of
already-ready on-chain requests.
...
3. **Strategy decision** — the strategy interface is `DecideOffers(ctx, OfferInput) (OfferOutput, error)`,
   ...
   unsafe full-offer yield. When `minAssetsPerRequest` is zero, the pinned protocol/E2E compatibility rule uses
   `max(2, ceil(principal / 1e6))` return units of margin instead of treating the disabled minimum as a
   one-base-unit request; this keeps ordinary auctions quotable while preserving the established partial-fill
   contract.
