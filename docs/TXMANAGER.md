...
| File | Responsibility |
|---|---|
| `txmanager.go` | public contract, process-owned state, serial worker, and synchronous/async send entry points |
| `admission.go` | lane demand, waiting, admission failures, and profitability fee query |
| `broadcast.go` | immutable signing and original/replacement/cancellation broadcast attempts |
| `pending.go` | the admitted transaction state machine and bounded shutdown |
| `replacement.go` | replacement ticks, exact rebroadcast, fee bumping, and same-nonce cancellation |
| `replacement_policy.go` | pure replacement and cancellation fee policy |
...
| `confirmation.go` | stable-head confirmation and canonical receipt-block proof |
| `helpers.go` | immutable request/fee copies and numeric/time helpers |

`manager_test.go` exercises the small set of safety contracts across those files rather than mirroring their
...
| Lifecycle occupancy | the same `Start` worker plus `admissionDemand` | `LaneReady` includes queued demand |
| Local nonce and runtime conflict | `Manager.mu` | a conflict pauses new external commitments |
| Active unmined lifecycle | worker-local state | manager cancellation is observed by the state machine |
| Readiness subscribers | `laneStateMu` | notifications carry no state; consumers re-read `LaneReady` |
| Result delivery | the serial worker | every admitted caller receives exactly one owned lifecycle result |
...
Receipt finality is proven by reading the receipt block through the canonical number mapping under a stable head. A
...
- Use `Send` for accepted obligations; admission waits without signing while the nonce lane is unavailable.
