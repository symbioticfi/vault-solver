...
|---|---|---|
| CLI composition, solver registration, or common config | [Architecture](ARCHITECTURE.md), [Development](DEVELOPMENT.md) | `cmd/vault-solver`, `internal/{config,solver}` |
...
| Strategy interface, default policy, or webhook decision | [Strategy contract](STRATEGIES.md), owning integration plan | `internal/solvers/<name>/strategies`, `internal/{liquidlane/strategies,webhook}` |
...
|---|---|---|
| Dependency direction and startup composition | [Architecture](ARCHITECTURE.md) | `cmd/vault-solver`, `internal/solver`, `internal/solvers` |
...
| Solver-local decision boundary | [Strategy contract](STRATEGIES.md) | `internal/solvers/*/strategies`, `internal/liquidlane/strategies`, `internal/webhook` |
