...
`cmd/vault-solver` is the composition root. Concrete solvers self-register from package `init` functions and
are blank-imported only by the command. Runtime startup is:
...
2. Register its factory in `init` and blank-import it from `cmd/vault-solver/root.go`.
