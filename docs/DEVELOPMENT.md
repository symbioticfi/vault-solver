...
make test-e2e-suite                             # compile the solver-owned tagged E2E package
make test-e2e E2E_PROFILE=rfq                   # run one pinned live profile
make test-e2e E2E_PROFILE=all                   # run every pinned live profile
```
...
Live E2E uses the pinned harness checkout initialized under ignored `.e2e/`; `make test-e2e-harness` verifies
the harness fixtures and orchestration without bringing up every protocol stack. Profiles are `3f`, `rfq`,
`lifi`, `uniswapx`, and `redstoneoev`. RFQ, LI.FI, and UniswapX each run external- and internal-LiquidLane
variants; 3F and OEV run their protocol path. Initialization applies the repository-owned image-isolation patch,
and `make test-e2e` derives a per-worktree image prefix so concurrent builds cannot replace the binary under test.
When another local stack is active, give a direct harness run a unique `E2E_IMAGE_PREFIX` and
`COMPOSE_PROJECT_NAME` and non-conflicting host ports rather than removing another worktree's containers.

## Change map

| Change | Required context | Focused check |
|---|---|---|
| Common config, schema, solver composition, or example | [Architecture](ARCHITECTURE.md), affected examples | `make verify-race TARGET='./internal/config ./internal/app ./cmd/vault-solver'` |
| CLI composition or process lifecycle | [Architecture](ARCHITECTURE.md) | `make verify-race TARGET='./internal/app ./cmd/vault-solver'` |
...
| Planner contract or webhook wire shape | [Strategy contract](STRATEGIES.md), owning integration plan | affected solver planner plus wire JSON tests |
